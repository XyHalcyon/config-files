#!/usr/bin/env bash
# =============================================================================
# generate-redhat.sh — RedHat 系 (openEuler/CentOS/Fedora/RHEL/Rocky) Dockerfile 生成器
#
...
RUN "
    ...
    for loc in "${larr[@]}"; do
        ...
        local charmap="${loc#*.}"
        if [[ "$first" -eq 1 ]]; then
            out+="localedef -i ${input} -f ${charmap} ${loc}"
            first=0
        else
            printf -v _seg " && \\\\\n    localedef -i %s -f %s %s" "$input" "$charmap" "$loc"
            out+="${_seg}"
        fi
    done
    out+=$'\n'
